...
def restore_checkpoint(opt,model,load_name=None,resume=False,best=False,evaluate=False):
    ...
    if resume:
        ...
        if evaluate:
            ...
            missing, unexpected = model.graph.load_state_dict(pretrained_dict,strict=False)
            print('Missing keys:')
            print('########################')
            print(missing)
            print('########################')
            ...
            model.graph.load_state_dict(checkpoint["graph"],strict=True)            
    else:
        checkpoint = torch.load(load_name,map_location=opt.device)
